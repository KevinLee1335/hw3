...
students[students == 4] <- 'Senior'
students[students == 5] <- 'Senior'
...
在這題，同學需要撰寫一個函數 ``，用來計算 `students2` 裡面各年級的比例。這個函數有兩個參數：
...
grade_percentage <- function(grades, students) {
   ...
   if (is.character(grades)){
     ...
     students[students == 3] <- 'Junior'
     students[students == 4] <- 'Senior'
     precentage <-  mean(students == grades)
     if(length(grades) > 1){
       precentage <- precentage + mean(students == grades[2])
     }
}
   else {
     precentage <- mean(students == grades)
